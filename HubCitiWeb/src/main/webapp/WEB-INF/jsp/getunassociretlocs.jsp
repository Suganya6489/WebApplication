...
<body onload="resizeDoc();" onresize="resizeDoc();">
	<!--Wrapper div Starts-->
		<div class="wrpr-cont relative">
			...
			<!--
			<input type="hidden" name="prevstate" value="${sessionScope.prevstate}" id="prevstate"/>
			...
			<div class="clear"></div>
							...
							<div class="tabd-pnl">
								<ul class="nav-tabs">
									<li><a  href="getassociretlocs.htm"
									title="Associated Locations" >Associated Locations</a></li>
									<li><a href="getunassociretlocs.htm" title="UnAssociated Locations" class="active rt-brdr" >UnAssociated Locations</a>
									</li>
								</ul>
								<div class="clear"></div>
							</div>
							<div class="cont-wrp">
									...
									<tr>
										...
										<td width="25%" align="left" valign="top">
											<div id="cityDiv" class="cntrl-grp zeroBrdr input-slct">
												...
												<span class="lesWdth"> <select name="city" title="Select City"
														path="city" class="slctBx" id="rtlr-city"
										...
										<td width="22%" rowspan="2">
											<div class="cntrl-grp zeroBrdr valgn-top input-mltislct"
												id="ZipcodeDiv">
												<span class="lesWdth"> <select name="postalCode" title="Select Zipcode"
														path="postalCode" id="multi-slct" multiple="multiple"
								...
								<c:choose>
								<c:when test="${sessionScope.unassocretlst.cityExpLst ne null && !empty sessionScope.unassocretlst.cityExpLst}">
										...
										<table width="100%" cellspacing="0" cellpadding="0" border="0"
											...
											<tbody class="scrollContent">
												<c:if
													test="${sessionScope.unassocretlst.cityExpLst ne null && !empty sessionScope.unassocretlst.cityExpLst}">
													<c:forEach items="${sessionScope.unassocretlst.cityExpLst}"
														var="retlst">
								...
								<div class="alertBx warning mrgnTop cntrAlgn" id="retClose">
									...
									<div class="row mrgnTop">
										...
										<div class="col" align="right">
											<input type="button" class="btn-blue" value="Associate" title="Associate Retailer"
												onclick="AssociateRetailer()" />
										</div>
										</c:if>
									</div>
								</div>
	...
	<script type="text/javascript">
		...
		if(alrtId == 'retClose'){
			...
			$(this).parent('div.alertBx').hide();
							...
							/*Check & uncheck all child chkbx on click of parent checkbox*/
							$(document).on(
									'click.chkAll',
									'#assctdLoctn',
									function() {
		...
		function loadZipcodes() {
			...
			var state = $('#rtlr-state').val();
			var sCity = document.SetupRetLocationForm.selCity.value;
			var vfinalCity;
		...
		function searchRetailer(event) {
			...
			document.SetupRetLocationForm.action = "getunassociretlocs.htm";
		...
		var chkd = $.makeArray($('input[name="retLocIds"]:checkbox:checked')
				...
				if(chkd == "")
				{
					...
					var assolist=chkd;
					assolist = $.trim(assolist);
			
				document.SetupRetLocationForm.unAssociRetLocId.value=assolist;
				document.SetupRetLocationForm.action = "associateret.htm";
